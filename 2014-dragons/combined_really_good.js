...
dragon = function(index) {
  ...
  var scale = 0.1 + index * index / 36, 
    ...
    speed = 3 + ran() * 5, 
    direction = 0, //ran() * pi * 2, //ran(0,TAU), 
  ...
  return function() {
    ...
    for (var i=0; i < 70; i++) {
      ...
      if (i) {
        ...
        snake[i].x -= dx * mod;
        snake[i].y -= dy * mod;
        snake[i].px = Mc(perpendicular);
        snake[i].py = Ms(perpendicular);
        ...
        if ( i == 20) {
         ...
         var wingPerpindicular = perpendicular;
    ...
    for (i=0; i < 156; i+=2) { // shape.length * 2 - it's symmetrical, so draw up one side and back down the other
      ...
      if (index >= 58) {  // draw tail
        ...
        var wobble = Math.sin(wobbleIndex / 3 + pfloat * 0.1) * wobbleIndex * L;
        ...
        // 6 is hinge point
        // x = 6 + (x-6) * (Ms((index + pfloat) * .04) + 2) * 2
        // x = 4 + (x-4) * (Ms((index + pfloat) * .04 * speed * 0.3) + 2) * 2
        // x = 4 + (x-4) * (Ms((index * 0.7 + pfloat) * .04 * speed * 0.3) + 2) * 2
        x = 4 + (x-4) * (Ms(( -x / 2 + pfloat) / 25 * speed / 4) + 2) * 2; // feed x into sin to make wings "bend"
        // x = 4 + (x-4) * (Ms(( Math.pow(1.1,x) * -0.03 + pfloat) * .04 * speed * 0.3) + 2) * 2

        basePosition.px = Mc(wingPerpindicular);
        basePosition.py = Ms(wingPerpindicular);// * (Ms((pfloat) * .04 * speed * 0.3) + 2) * 2; 
...
// last few digits of string shape are no longer needed.
// first two digts probably not needed either.
